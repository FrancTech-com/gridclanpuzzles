...
public interface PlayerStatsRepository extends JpaRepository<ScrabbleGame, UUID> {
    ...
    @Query(value = """
        ...
        FROM (
            SELECT 'SCRABBLE'   AS game_type, id, player1_id, player2_id, winner_id, vs_computer, status FROM scrabble_games
            UNION ALL
            SELECT 'GOMOKU'     AS game_type, id, player1_id, player2_id, winner_id, vs_computer, status FROM gomoku_games
            UNION ALL
            SELECT 'BATTLESHIP' AS game_type, id, player1_id, player2_id, winner_id, vs_computer, status FROM battleship_games
        ) g
