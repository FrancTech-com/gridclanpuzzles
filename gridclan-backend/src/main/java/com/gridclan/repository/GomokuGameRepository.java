...
public interface GomokuGameRepository extends JpaRepository<GomokuGame, UUID> {
    ...
    List<GomokuGame> findByStatus(String status);

    /** The user's in-progress games (either seat), most-recently-active first. */
    @Query("SELECT g FROM GomokuGame g WHERE (g.player1Id = :uid OR g.player2Id = :uid) "
         + "AND g.status IN :statuses ORDER BY g.lastMoveAt DESC")
    List<GomokuGame> findResumable(@Param("uid") UUID uid, @Param("statuses") Collection<String> statuses);
}
